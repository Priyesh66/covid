...
class CovidPatientRepository extends PatientRepository {
  ...
  Future<Map<String, List>> fetchPatientData() async {
    ...
    if (res.statusCode == 200) {
      ...
      stateDataList =
          stateWise.map<MyStateData>((json) => MyStateData.fromJson(json))
              .toList();
  ...
  @override
  Future<Map<String, List<MyStateSingleValue>>> fetchStatePatientDailyData(
    ...
    var stateDailyRes = await http.get(
        Uri.encodeFull(stateDailyDataUrl), headers: {
      "Accept": "appplication/json",
    });
    ...
    if (stateDailyRes.statusCode == 200) {
      ...
      var confirmedDailyData = dailyStateData.where((item) =>
      item["status"] == "Confirmed").toList();
      var recoveredDailyData = dailyStateData.where((item) =>
      item["status"] == "Recovered").toList();
      var deathsDailyData = dailyStateData.where((item) =>
      item["status"] == "Deceased").toList();
      ...
      for (int index = 0; index < cnfStateDataList.length; index++) {
        ...
        MyStateSingleValue rcvrdStateData = rcvrdStateDataList.firstWhere((
            item) => item.dateEquals(cnfStateData.dateString));
        MyStateSingleValue deathsStateData = deathsStateDataList.firstWhere((
            item) => item.dateEquals(cnfStateData.dateString));
        MyStateSingleValue activeStateData = MyStateSingleValue(
            stateCode: stateCode,
            status: "Active",
            dateString: cnfStateData.dateString,
            value: (cnfStateData.value - rcvrdStateData.value -
                deathsStateData.value));
        ...
        cnfStateCumulativeDataList.add(MyStateSingleValue(stateCode: stateCode,
            status: "Confirmed",
            dateString: cnfStateData.dateString,
            value: cumulativeCnfValue));
        rcvrdStateCumulativeDataList.add(MyStateSingleValue(
            stateCode: stateCode,
            status: "Recovered",
            dateString: cnfStateData.dateString,
            value: cumulativeRcvrdValue));
        deathsStateCumulativeDataList.add(MyStateSingleValue(
            stateCode: stateCode,
            status: "Deaths",
            dateString: cnfStateData.dateString,
            value: cumulativeDeathsValue));
        activeStateCumulativeDataList.add(MyStateSingleValue(
            stateCode: stateCode,
            status: "Active",
            dateString: cnfStateData.dateString,
            value: cumulativeActiveValue));
      }

      print("List is: " + activeStateDataList.toString());

      statePatientDataMap["cnf_state_cumulative_data"] =
          cnfStateCumulativeDataList.reversed.toList();
      statePatientDataMap["rcvrd_state_cumulative_data"] =
          rcvrdStateCumulativeDataList.reversed.toList();
      statePatientDataMap["deaths_state_cumulative_data"] =
          deathsStateCumulativeDataList.reversed.toList();
      statePatientDataMap["active_state_cumulative_data"] =
          activeStateCumulativeDataList.reversed.toList();
    } else {
  ...
  @override
  Future<List<MyStateData>> fetchDistrictWiseData(String stateCode) {
    // TODO: implement fetchDistrictWiseData
